...
def main(args):
    ...
    if not os.path.exists(
        ...
        for surefire_report in surefire_reports:
            ...
            with open(
                f"{surefire_report_path}/{surefire_report}", "r", encoding="utf-8"
            ) as f:
                data = f.read()
            ...
            with open(coverage_report_path, "r") as f:
                ...
                for package in root.findall("package"):
                    for class_ in package.findall("class"):
                        coverage_details[test_class][test_case].setdefault(
                            class_.attrib["name"], []
                        )
                        ...
                        for method in class_.findall("method"):
                            for counter in method.findall("counter"):
                                if (
                                    counter.attrib["type"] == "METHOD"
                                    and counter.attrib["covered"] != "0"
                                ):
                                    coverage_details[test_class][test_case][
                                        class_.attrib["name"]
