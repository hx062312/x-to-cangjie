...
def get_invoked_expressions(code, node):
    ...
    def build_expression(node):
        ...
        if node.type == "method_invocation":
                        ...
                        parts.append(
                            "".join(
                                extract_text_by_bytes(
                                    code, arg.start_byte, arg.end_byte
                                )
            ...
            return extract_text_by_bytes(
                code, node.start_byte, node.end_byte
            ) + "".join(build_expression(child) for child in node.children)
        else:
...
def has_method_invocation(code, node, main_application_methods):
    ...
    if node.type == "method_invocation":
        ...
        for method_name in method_names:
            if (
                method_name in main_application_methods
                and f"{method_name}("
                in extract_text_by_bytes(code, node.start_byte, node.end_byte)
            ):
    ...
    for child in node.children:
        child_found, child_application_methods, child_invocation_expressions = (
            has_method_invocation(code, child, main_application_methods)
        )
    ...
    if node.type == "method_declaration":
        ...
        if block_node:
            for statement in block_node.children:
                ...
                if statement.type in [
                ...
                ] or statement.type.endswith("statement"):
                    ...
                    status, invoked_methods, invoked_expressions = (
                        has_method_invocation(code, statement, main_application_methods)
                    )

                    # sort invoked expressions based on length
                    invoked_expressions = list(set(invoked_expressions))
                    invoked_expressions = sorted(
                        invoked_expressions, key=lambda x: len(x)
                    )
                    ...
                    for expression in invoked_expressions[:last_index]:
                        if "@Override" in extract_text_by_bytes(
                            code, statement.start_byte, statement.end_byte
                        ):
                            continue
                        if statement.type in [
                            "try_statement",
                        ...
                        if len(set(invoked_methods)) == 1:
                            continue
                        if f"-> {expression}" in extract_text_by_bytes(
                            code, statement.start_byte, statement.end_byte
                        ):
                            continue
                        if "public void" in extract_text_by_bytes(
                            code, statement.start_byte, statement.end_byte
                        ):
                            continue
                        ...
                        statements.append(
                            {
                                ...
                                "constructor_call": False,
                                "assert_statement": False,
                                "invoked_methods": set(expression_invoked_methods),
                            }
                        )
...
def is_test_method(test_file, node, code):
    ...
    if node.type == "method_declaration":
        ...
        method_name = extract_text_by_bytes(
            code, method_name_node.start_byte, method_name_node.end_byte
        )
        test_class_path = (
            test_file[test_file.find("src/test/java/") + 14 :]
            .replace("/", ".")
            .replace(".java", "")
        )
        ...
        if sibling.type == "modifiers":
            for annotation in sibling.children:
                if annotation.type in ["marker_annotation", "annotation"]:
                    annotation_text = extract_text_by_bytes(
                        code, annotation.start_byte, annotation.end_byte
                    )
        ...
        if sibling.type == "modifiers":
            for annotation in sibling.children:
                if annotation.type in ["marker_annotation", "annotation"]:
                    ...
                    if (
                        "@Disabled" in annotation_text.strip()
                        or "@EnabledOnOs(OS.WINDOWS)" == annotation_text.strip()
                    ):
                        return True
    return False
    ...
    for child in node.children:
        if child.type == "throws":
            for exception in child.children:
                throws.append(
                    extract_text_by_bytes(
                        code, exception.start_byte, exception.end_byte
                    )
                )
    return throws
...
def find_test_methods(test_file, node, code, main_application_methods):
    ...
    for child in node.children:
        ...
        if (
            ...
            and is_test_method_status
            and not is_disabled_method(child, code)
        ):
            ...
            statements = extract_method_statements(
                child, code, main_application_methods
            )
            methods[method_name] = {
                "statements": statements,
                "throws": throws,
                "annotation": "@Test" if annotation_text is None else annotation_text,
            }
        # Recursively search within nested nodes
        methods.update(
            find_test_methods(test_file, child, code, main_application_methods)
        )
    return methods
...
def generate_new_test_methods(method_name, statements_throws, ignore=False):
    ...
    while i < len(statements_throws["statements"]):
        ...
        # Rule 4: Handle method_invocation
            ...
            while (
                ...
                and statements_throws["statements"][j]["invoked_methods"]
                == invoked_methods_set
            ):
        ...
        # Rule 3: Combine standalone statements with subsequent statements
            ...
            while j < len(statements_throws["statements"]):
                ...
                if (
                    next_statement["assert_statement"]
                    or next_statement["constructor_call"]
                    or next_statement["method_invocation"]
                ):
    ...
    for i, statement_group in enumerate(gobbled_method_statements):
        ...
        accumulated_statements = [
            x["text"] for group in gobbled_method_statements[: i + 1] for x in group
        ]
...
def find_ignored_test_methods(test_file, node, code):
    ...
    for child in node.children:
        ...
        if (
            child.type == "method_declaration"
            and is_ignore_method(child, code)
            and is_test_method_status
        ):
            method_name_node = child.child_by_field_name("name")
            method_name = extract_text_by_bytes(
    ...
    for child in node.children:
        if child.type == "class_declaration":
            ...
            if superclass_node:
                ...
                superclass_name = extract_text_by_bytes(
                    code, superclass_node.start_byte, superclass_node.end_byte
                )
...
def get_class_method_names(node, code):
    ...
    for child in node.children:
        if (
            child.type == "method_declaration"
            or child.type == "constructor_declaration"
        ):
            method_name_node = child.child_by_field_name("name")
            method_name = extract_text_by_bytes(
                code, method_name_node.start_byte, method_name_node.end_byte
    ...
    for src_file in src_files:
        ...
        for node in tree.root_node.children:
            if node.type == "class_declaration":
                ...
                class_name = extract_text_by_bytes(
                    java_code, class_name_node.start_byte, class_name_node.end_byte
                )
...
def main(args):
    ...
    for test_file in test_files:
        ...
        ignored_test_methods[test_file] = find_ignored_test_methods(
            test_file, root_node, java_code
        )

        # Extract test methods and their statements
        test_methods = find_test_methods(
            test_file, root_node, java_code, main_application_methods
        )
        ...
        removed_method_bodies = get_test_method_bodies(
            root_node, java_code, test_methods
        )
        ...
        (
            new_java_code.insert(import_line_index + 1, "import org.junit.Test;")
            if (
                "import org.junit.Test;" not in "\n".join(new_java_code)
                and "import org.junit.jupiter.api.Test;" not in "\n".join(new_java_code)
            )
            and "@Test" in "\n".join(new_java_code)
            else None
        )
    ...
    for test_file in ignored_test_methods:
        ...
        super_class_test_file = [
            f for f in test_files if f.endswith(f"{super_class}.java")
        ][0]

        for ignored_method in ignored_methods_actual_name:
            for i in range(
                len(decomposed_method_details[super_class_test_file][ignored_method])
            ):
                ...
                java_code = (
                    java_code[: java_code.rfind("}")] + new_ignored_method_body + "}"
                )
