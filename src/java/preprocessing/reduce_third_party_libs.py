        ...
        for file in files:
            if file.endswith(".java"):
                ...
                if "/java/" not in file_path:
                    continue
                class_path = (
                    file_path.split("/java/")[1].split(".")[0].replace("/", ".")
                )
                    ...
                    for i, line in enumerate(lines):
                        ...
                        for j in range(max_multiline):
                            if j > 0 and (i + j) < len(lines):
                                multiline += lines[i + j]
                            if (
                                re.match(class_regex, multiline.strip())
                                and inside_outer_class
                            ):
                                inner_class_match = re.match(
                                    class_regex, multiline.strip()
                                )
                            ...
                            if re.match(class_regex, multiline.strip()) and (
                                not inside_outer_class
                            ):
...
def identify_third_party_packages(source_code_path, current_project_packages):
    ...
    for root, _, files in os.walk(source_code_path):
        for file in files:
            if file.endswith(".java"):
                file_path = os.path.join(root, file)
                project_specific_file_path = (
                    os.getcwd().split("/")[-1] + "/" + file_path.split("/", 1)[-1]
                )
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    lines = f.readlines()
                for line in lines:
                    ...
                    import_match = re.match(
                        r"import\s+(static\s+)?([\w.]+(\.\*)?);", line.strip()
                    )
                    if import_match:
                        ...
                        if not any(
                            imported.startswith(std) for std in standard_java_packages
                        ):
                            if not any(
                                imported.startswith(pkg)
                                for pkg in current_project_packages
                            ):
...
def get_implemented_methods_and_child_classes_of_third_party_types(
    source_code_path, class_package_map, third_party_packages, third_party_exceptions
):
                ...
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        for j in range(max_multiline):
                            ...
                            if (
                                ...
                                if inner_class_match:
                                    ...
                                    interfaces_implemented = (
                                        inner_class_match.group(7)
                                        .split("implements")[1]
                                        .split(",")
                                        if inner_class_match.group(7) != None
                                        else []
                                    )
                                    interfaces_implemented_cleaned = [
                                        interface.strip().split("<")[0]
                                        for interface in interfaces_implemented
                                    ]
                                    classes_extended = (
                                        inner_class_match.group(6)
                                        .split("extends")[1]
                                        .split(",")
                                        if inner_class_match.group(6) != None
                                        else []
                                    )
                                    classes_extended_cleaned = [
                                        clazz.strip().split("<")[0]
                                        for clazz in classes_extended
                                    ]
                                    for (
                                        cleaned_interface
                                    ) in interfaces_implemented_cleaned:
                                        if cleaned_interface in class_package_map:
                                            if (
                                                not class_path
                                                in interface_implementation_map
                                            ):
                                                interface_implementation_map[
                                                    class_path
                                                ] = set()
                                            interface_implementation_map[
                                                class_path
                                            ].add(class_package_map[cleaned_interface])
                                            if (
                                                class_package_map[cleaned_interface]
                                                in third_party_packages
                                            ):
                                                child_classes_of_third_party_types.add(
                                                    class_path
                                                )
                                    for cleaned_class in classes_extended_cleaned:
                                        if cleaned_class in class_package_map:
                                            if (
                                                class_package_map[cleaned_class]
                                                in third_party_packages
                                            ):
                                                child_classes_of_third_party_types.add(
                                                    class_path
                            ...
                            if re.match(class_regex, multiline.strip()) and (
                                ...
                                if outer_class_match:
                                    ...
                                    match_some_header = True
                                    interfaces_implemented = (
                                        outer_class_match.group(7)
                                        .split("implements")[1]
                                        .split(",")
                                        if outer_class_match.group(7) != None
                                        else []
                                    )
                                    interfaces_implemented_cleaned = [
                                        interface.strip().split("<")[0]
                                    ...
                                    classes_extended = (
                                        outer_class_match.group(6)
                                        .split("extends")[1]
                                        .split(",")
                                        if outer_class_match.group(6) != None
                                        else []
                                    )
                                    classes_extended_cleaned = [
                                        clazz.strip().split("<")[0]
                        ...
                        for k in range(max_multiline):
                            ...
                            method_match = re.match(method_regex, multi_lines.strip())
                            if method_match:
                                method_signature = method_match.group(5)
                                full_method_name = (
                                    class_base_path
                                ...
                                if (
                                    not full_method_name
                                    in implemented_methods[class_path]
                                ):
                                    implemented_methods[class_path].append(
                                        full_method_name
                                    )
                                break
                            abstract_method_match = re.match(
                                abstract_method_regex, multi_lines.strip()
                            )
                            if abstract_method_match:
                                abstract_method_signature = abstract_method_match.group(
                                    5
                                )
                                full_abstract_method_name = (
                                    class_base_path
                                    + (
                                        ("$" + class_path.split("$")[1])
                                        if ("$" in class_path)
                                ...
                                if (
                                    not full_abstract_method_name
                                    in abstract_method_map[class_path]
                                ):
                                    abstract_method_map[class_path].append(
                                        full_abstract_method_name
                                    )
                                break
                        ...
                        for ex in third_party_exceptions:
                            if (ex in line) and (full_method_name is not None):
                                if not (
                                    ...
                                    if (
                                        class_path
                                        in methods_using_third_party_exceptions
                                    ):
                                        methods_using_third_party_exceptions[
                                            class_path
                                        ].add(full_method_name)
                                    else:
                                        exs = set()
    ...
    with open(callgraph_file, "r") as f:
        for line in f:
            ...
            if line.startswith("M:"):
                ...
                caller_method = (
                    caller.split(":")[1] + ":" + caller.split(":")[2].split("(")[0]
                )
                caller_method = re.sub(
                    r"\$\d+", "", re.sub(r"lambda\$", "", caller_method)
                )
                callee_method = callee.split(")")[1].split("(")[0]
                callee_method = re.sub(
                    r"\$\d+", "", re.sub(r"lambda\$", "", callee_method)
                )
                call_graph.append((caller_method, callee_method))
                caller_params = caller.split("(")[1].split(")")[0].split(",")
                callee_params = (
                    callee.split(")")[1].split("(")[1].split(")")[0].split(",")
                )
...
def identify_third_party_dependencies(
    call_graph,
    ...
    for third_party_ex_dependent_cls in third_party_exception_dependencies:
        for third_party_ex_dependent_method in third_party_exception_dependencies[
            third_party_ex_dependent_cls
        ]:
    ...
    for caller_method, callee_method in call_graph:
        ...
        if any(
            (
                callee_class.startswith(pkg)
                or (callee_method.replace(":", ".").startswith(pkg))
            )
            for pkg in third_party_packages
        ) and (
            ("<init>" in caller_method)
            or ("<clinit>" in caller_method)
            or ("setUp" in caller_method)
            or ("tearDown" in caller_method)
        ):
        ...
        for caller_method, callee_method in call_graph:
            ...
            if (callee_method == method) or (caller_class in classes_to_remove):
                if caller_class in dependent_class_methods:
                    ...
                    methods.add(caller_method)
                    dependent_class_methods[caller_class] = methods
                if (not caller_method in examined) and (
                    not caller_method in to_examine
                ):
                    to_examine.append(caller_method)
        ...
        for caller_method, callee_method in call_graph:
            ...
            for callee_param in method_param_map[callee_method]:
                if (callee_param in classes_to_remove) or (
                    callee_param in third_party_packages
                ):
                    if not any(
                        callee_class.startswith(std) for std in standard_java_packages
                    ):
                        ...
                        if (not callee_method in examined) and (
                            not callee_method in to_examine
                        ):
            ...
            for caller_param in method_param_map[caller_method]:
                if (caller_param in classes_to_remove) or (
                    caller_param in third_party_packages
                ):
                    if caller_class in dependent_class_methods:
                        dependent_class_methods[caller_class].add(caller_method)
                    else:
...
def refactor_code(
    source_code_path, dependent_class_methods, third_party_packages, classes_to_remove
):
    classes_names_to_remove = [
        path.replace("$", ".").split(".")[-1] for path in classes_to_remove
    ]
                            ...
                            for i, line in enumerate(lines):
                                ...
                                if import_match:
                                    ...
                                    if (
                                        (
                                            any(
                                                imported.startswith(third_pkg)
                                                for third_pkg in third_party_packages
                                            )
                                        )
                                        or (
                                            any(
                                                ...
                                                for dep_method in third_party_packages
                                            )
                                        )
                                        or (
                                            any(
                                        ...
                                        test_class_dependent_on_third_party_fields = (
                                            True
                                        )
                                        break
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if import_match:
                            ...
                            if (
                                ...
                                or (
                                    any(
                                        (imported == class_to_remove.replace("$", "."))
                                        for class_to_remove in classes_to_remove
                                    )
                                )
                                or (
                        ...
                        for j in range(max_multiline):
                            ...
                            if j == 0:
                                if (
                                    re.match(r"\s*@\w+(\([^()]*\))?", multiline.strip())
                                    and not inside_method
                                ):
                                    ...
                                    if i + j < len(lines):
                                        ...
                                        for k in range(max_multiline - 1):
                                            ...
                                            if method_match:
                                                ...
                                                exceptions_thrown = (
                                                    method_match.group(7)
                                                    .split("throws")[1]
                                                    .split(",")
                                                    if method_match.group(7) != None
                                                    else []
                                                )
                                                exceptions_thrown_cleaned = [
                                                    exception.strip()
                                                    for exception in exceptions_thrown
                                                ]
                                                if (
                                                    full_method_name
                                                    in methods_to_remove
                                                ) or any(
                                                    ex in classes_names_to_remove
                                                    for ex in exceptions_thrown_cleaned
                                                ):
                                                    match_some_header = True
                                                    annotation_buffer = []
                                                    annotation_cached = False
                            ...
                            method_match = re.match(method_regex, multiline.strip())
                            if (not inside_method) and method_match:
                                ...
                                exceptions_thrown = (
                                    method_match.group(7).split("throws")[1].split(",")
                                    if method_match.group(7) != None
                                    else []
                                )
                                exceptions_thrown_cleaned = [
                                    exception.strip() for exception in exceptions_thrown
                                ]
                                inside_method = True
                                skip_method = (
                                    full_method_name in methods_to_remove
                                ) or (
                                    any(
                                        ex in classes_names_to_remove
                                        for ex in exceptions_thrown_cleaned
                                ...
                                method_brace_level = (
                                    0  # Start counting from the opening brace
                                )
                                match_some_header = True
                                skip_lines += j
                ...
                with open(file_path, "r", encoding="utf-8", errors="ignore") as file:
                    ...
                    for class_name in classes_removed:
                        if re.search(r"extends\s+" + re.escape(class_name), content):
                            print(
                                f"Deleting file (depends on removed class): {file_path}"
                            )
                            os.remove(file_path)
                            break
...
def main(project_name):
    ...
    class_package_map, third_party_packages, third_party_exceptions = (
        identify_third_party_packages(source_code_path, current_project_packages)
    )
    (
        child_classes_of_third_party_types,
        implemented_methods,
        ...
        interface_implementation_map,
        abstract_method_map,
    ) = get_implemented_methods_and_child_classes_of_third_party_types(
        source_code_path,
        class_package_map,
        third_party_packages,
        third_party_exceptions,
    )
    call_graph, method_param_map, junit_pionner_dependent_classes = parse_callgraph(
        "callgraph.txt"
    )
